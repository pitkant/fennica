...
#' source. Otherwise, a readily preprocessed version from the fennica R
#' package is used.
...
#' The data was downloaded with this function as fennica::get_gender_fi()
#' and integrated in the fennica package. The frequencies of male/female names
#' are added after the download as some names occur for both genders.
#' @references See cite("fennica")
#' @keywords data
get_gender_fi <- function (download = FALSE) {
  ...
  if (download) {
    ...
    f <- system.file("extdata/gender_FI.csv", package = "fennica")
