...
get_publishers_Finto <- function(Finto_corrected, Finto_comp, all_names, known_inds, Finto_town, df, Finto_years) {
  ...
  all_data$ignore[known_inds] <- TRUE
  ...
  for (idx in 1:nrow(unique_data)) {
    ...
    all_names_indices <- intersect(all_names_indices, which(df$publication_place==unique_data$town[idx]))
    ...
    # Why this is here with for loop? - I comment it out now. / LL
    # idx <- idx + 1
  }
