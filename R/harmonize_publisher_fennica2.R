...
harmonize_publisher_fennica2 <- function(x, publication_year, languages=c("english")) {
  ...
  L <- length(unique(q))
  ranges <- data.frame(min=integer(L), max=integer(L), publisher=character(L), stringsAsFactors=FALSE)
  ...
  for (pub in unique(q)) {
    ...
    max_year <- max(publication_year$publication_year_till[inds], na.rm = TRUE)
    if (is.na(pub)) {
      next
    }
  ...
  # This could be outside for loop as well? Moving there.
  ...
  for (publisherName in na.omit(tab[,1])) {
    ...
    publisherTotal <- unlist(subset(tab, tab[,1]==publisherName)[2], use.names=FALSE)
    ...
    inds2 <- ranges$publisher==publisherName
    pub_start <- ranges$min[inds2]
    pub_end <- ranges$max[inds2]
