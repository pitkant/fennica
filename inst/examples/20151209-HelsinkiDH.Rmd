...
```{r 20151209-HDH-topplace, echo=FALSE, message=FALSE, warning=FALSE, cache=FALSE, fig.height=4, fig.width=5}
...
```{r 20151209-HDH-topg, echo=FALSE, message=FALSE, warning=FALSE, cache=FALSE, fig.height=6, fig.width=9}
...
p <- ggplot(dfs, aes(x = publication_decade, y = n, color = gatherings)) + geom_point(size = 3) + geom_smooth(fill = "lightgray", size = 2, alpha = 0.2) + ggtitle("Top gatherings timeline") + ylim(c(0, max(dfs$n)))
print(p)
```