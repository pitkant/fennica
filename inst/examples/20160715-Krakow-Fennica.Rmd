...
knitr::opts_chunk$set(fig.path = "figure-20160715-Krakow-Fennica/")
...
df <- df %>% filter(!is.na(period)) %>% group_by(period, paper.consumption.km2)
df <- df %>% summarise(n = n(),
		  paper2 = sum(paper.consumption.km2, na.rm = TRUE)) %>%
             mutate(paper.per.title = paper2/n)		 
df$paper.consumption.km2 <- df$paper2
df$paper2 <- NULL
...
for (catal in unique(df0$catalog)) {
  df2 <- df0 %>% filter(catalog == catal) %>% 
		 ...
		 summarise(paper2 = sum(paper.consumption.km2, na.rm = TRUE), n = n()) 
