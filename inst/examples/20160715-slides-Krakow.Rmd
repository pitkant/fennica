...
- Fennica (Finnish National Bibliography): `r nrow(filter(df0, catalog == "Fennica" & publication_year >= 1470 & publication_year <= 1828))` documents

- Kungliga (National Library of Sweden): `r nrow(filter(df0, catalog == "Kungliga" & publication_year >= 1470 & publication_year <= 1828))` documents
...
--> Deep roots of the diversity of "Finnishness" reflected in knowledge production, 1640-1828.
...
Selitys puuttuville palloille: Kungligassa ei ole yhtään kaupunkia mainittuina vuosikymmeninä joille geokoordinaatit olisi tiedossa. Eli dataa ei ole. Ei siis palloja. Johtunee siitä että ESTC:n ja Fennican osalta näitä joskus aiemmin selviteltiin mutta Kungligan ruotsalaisille paikannimille ei ole haettu geokoordinaatteja eikä niitä juuri näköjään ole aiemmin käyttämissämme geokoordinaattiaineistoissa. Pitää joko vaihtaa takaisin Fennicaan tässä, tai toivoa että ehditään etsiä ja soveltaa ruotsalaisia geokoordinaattijuttuja ajoissa. 

Voi miettiä, jos tämä korvaisi alla tulevan "Comparing publishing activity in Swedish University town".
...
TODO: iso piikki Kungligassa (1760-1770) ja kaksi laskua Fennicassa (1700-1721) ja (1809-1828). Voisko ne HIGHLIGHTata jollain tyylikkäällä tavalla? Tai sit joku päivä voi kehitellä paneeleita tms. Lisäsäätö vie aikaa, pitäis vähän tutkiskella asiaa.
...
TODO: Tässä vois maalata octavo-kirjojen käyrän molemmista kuvista kirkkaalla värillä (ei muuta).
...
Proportions for title count, paper, and publishers
TODO: OTHER julkaisijakenttä laittaa ton Turku-kuvan pieleen, "other" kategoria liian iso. LL: eli ohjeet mitä tehdään ?
```{r comparisons, echo=FALSE, message=FALSE, warning=FALSE, echo=FALSE, fig.width=20, fig.height=8}
pics <- list()
...
TODO: Voisko tämän ja seuraavan kuvan (ruotsalaiset julkaisijat) jotenkin yhdistää järkevästi selvyyden vuoksi? En valitettavasti tiedä miten se teknisesti tehtäisiin.
...
[Swedish Age of Freedom and Abolishment of Censorship TODO]

### TODO 1766-1772

Kungligan mukaan piirakkakuva prosentteina (tai joku vastaava, vaihtelun vuoksi) kuinka paljon enemmän Kungligassa julkaisujen lukumäärä ja paperinkulutus kasvaa kun sensuuri puuttuu vuosina 1766-1772 suhteessa tätä edeltävien (esim. kahden) vuosikymmenen kasvuun. Eli siis vuodet 1766-1772 ja kuinka paljon silloin on enemmän julkaisuja sensuurin puuttumisen takia tarkoituksena selvittää. Eli odotettu kasvu (kun katsotaan käyrää Kungligasta se on aika tasainen näiden vuosien ympärillä ylöspäin) ja todellinen kasvu (piikki) sekä niiden ero prosentteina.

LL: no ainakin data nyt tässä laskettuna kahdelle peräkkäiselle 6 vuoden jaksolle. Tehdään ensin analyysit jotka tarvitaan. Hiotaan kuvia lisää sitten kun ei tule enää muuten lisää kamaa. 
...
TODO: Tästä kaikki Fennica kuvat pois analyysistä selkeyden vuoksi.
...
###TODO: asiasanoja / yliopistopaikkakunta vertailu. Ts. mitä enemmän eri aikoina asiasanoja (ihan sama mitä ne on) paikkakunnalla, sitä monimuotoisempaa/rikkaampaa julkaisutoiminta (eli kaikki julkaisut eivät samasta aiheesta, esim. oikeuspöytäkirjoissa ei varmaan hirveästi vaihtelua). Voidaan suhteuttaa myös esim. paperinkulutukseen. Greifswald vs Turku (kuinka monta kieltä per vuosi) oli samansuuntainen. 

---

### Languages

TODO: "Other" kielikategorian selvittäminen seuraavissa kuvissa. Onko siellä joku tietty kieli joka näkyy hyvin selvästi mukana analyysissä? Nousee isoksi kategoriaksi. Nyt oon merkannut, että Kreikan kieli (Greek - huom on kielet Greek, Modern (1453-) sekä Greek, Ancient (to 1453) mutta ei pelkka Greek) mukaan moneen kohtaan. Jos kyseessä joku muu kuin Kreikka niin sitten se. Jos taas paljon kombinaatioita pieniä kieliä, niin antaa siinä tapauksessa olla. LL: Lisäsin Kreikan mutten muuten ehdi tutkia asiaa. Voit katsoa R:llä top(df0, "language", 10)
...
catalogue <- "Fennica"
df <- df0
langs <- c("Finnish", "Swedish", "Latin", "German", "Russian", "French", "Greek, Ancient (to 1453)", "Other")
...
### Different languages in Turku, Stockholm and Uppsala
TODO: Rajataan analyysi Turku (Fennica), Stockholm (Kungliga) ja Uppsala (Kungliga)
```{r language-perc2, echo=FALSE, message=FALSE, warning=FALSE, fig.width=20, fig.height=5}
pics <- list()
theme_set(theme_bw(20))
...
Yleinen kielianalyysi prosentteina (sama kuin muut toteutetut prosenttianalyysit näissä Krakova slideissa) kaikkien Fennicassa väitöskirjoiksi merkityistä: eli Kenttä "Dissertation" ja vastaus "True" tässä aineisto. Analyysi vuosikymmenittäin ja kielen mukaan eri väreillä. Kielien joukko: 
...
TODO: kuva rajattu 1640-1828, MUTTA piirtää silti tuonne nuo tyhjät vuodet 1470-1640. NE PITÄIS SAADA PRINTISTÄ POIS.
...
Kaupungit: "Tukholma", "Lund", "Greifswald", "Uppsala"
