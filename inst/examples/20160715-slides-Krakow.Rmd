...
## Global knowledge production and large-scale library catalogues
...
**Overcoming a national gaze** focus on knowledge production through more complex cross-national networks

**Data: bibliographic catalogues 1640-1828**

- Fennica: `r nrow(subset(df0, catalog == "Fennica" & publication_year >= 1640 & publication_year < 1828))` documents
- Kungliga: `r nrow(subset(df0, catalog == "Kungliga" & publication_year >= 1640 & publication_year < 1828))` documents
...
- **Automated** scales well with new data and unforeseen changes

- **Reusable** the polished data can be reused and further enriched

- **Customized** dedicated R packages for bibliographic analysis combine automation with flexibility
...
- **Parsers** From original MARC/XML to CSV to RData 

- **Preprocessing** Cleaning up the fields. 

```{r}
library(bibliographica)
polish_physical_extent("iii-xxiv, 118, [2] p.")$pagecount
polish_place("Sur l'imprimea londres")
```
...
```{r}
library(knitr)
kable(as.data.frame(top(df0, "publication_place", 3)))
```
...
**Quantitative Study of Finnish Knowledge Production, 1640-1828**
...
TODO Karttakuvassa Greifswald, Lund ei näy. Tätä hyvä saada kehitettyä, tällaisena ei toimi. LL: ei näy sen takia että näytettynä on vuosikymmen 1650 jolloin Lundissa on 0 dokumenttia ja Greifswaldissa 1. Muissa kymmeniä tai satoja. Data näkyy kuten pitääkin.

```{r map1, echo=FALSE, message=FALSE, warning=FALSE, fig.width=20, fig.height=5, eval=FALSE}
library(dplyr)
library(ggmap)
library(ggplot2)
library(gisfin)
...
```{r publishingactivitycomparisons, echo=FALSE, message=FALSE, cache=TRUE, fig.width=8, fig.height=5, fig.show="hold", out.width="150px"}
v <- seq(1500, 1800, 100)
...
TODO: Vaihdettiin tähän Stockholm, riittää julkaisumäärät. 
...
p <- ggplot(npub, aes(x = publication_decade, y = n)) +
       ...
       ggtitle(paste("Unique publishers in selected publication places"))
...
for (catalogue in c("Fennica", "Kungliga")) {
  ...
  langs <- c("Finnish", "Swedish", "Latin", "German", "Russian", "French", "Danish", "English", "Other")
  df <- df %>% filter(catalog == catalogue)%>%
        filter(publication_year >= 1640 & publication_year <= 1900)
...
[University curriculum & Enlightenment in Finland]
TODO: UUSI SLIDE top-authors publishing in octavo (title count) Fennica and Kungliga. OF COURSE!
...
- https://github.com/rOpenGov/fennica/blob/master/inst/examples/20160715-slides-Krakow.Rmd
