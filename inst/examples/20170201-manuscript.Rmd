...
```{r Fig3prep, echo=FALSE, message=FALSE, warning=FALSE, echo=FALSE, fig.width=18, fig.height=5, fig.show="hide"}
library(scales)
...
```{r Fig3, echo=FALSE, message=FALSE, warning=FALSE, echo=FALSE, fig.width=18, fig.height=5, fig.show="hide"}
...
for (catalogue in c("Fennica", "Kungliga")) {
  ...
  brs <- seq(min(dfss$publication_decade), max(dfss$publication_decade), 10)
   ...
   p <- ggplot(dfss,
      ...
      #scale_fill_stata() +
      scale_fill_manual(values = lang.colors[as.character(levels(dfss$publication_place))]) +      
      scale_x_continuous(breaks = brs, labels = insert_minor(brs, 5)) +  
      #scale_fill_brewer(palette="Greys") +       
