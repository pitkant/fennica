...
for (catal in unique(df$catalog)) {
  pics[[catal]] <- ggplot(subset(df, catalog == catal),
       ...
       ggtitle(paste("Average title length (", catal, ")", sep = "")) + 
       xlab("Year") +
...
p <- ggplot(npub, aes(x = publication_decade, y = n)) +
       ...
       scale_fill_stata() +
       scale_x_continuous(breaks = brs,
           labels = insert_minor(brs, 5, start = min(npub$publication_decade))) + 
       guides(fill = guide_legend(reverse = FALSE, title = "")) +
       ggtitle(paste("Publishers (", catal, ")", sep = ""))       
...
p <- ggplot(df, aes(x = publication_year, y = n)) +
     ...
     ggtitle("") +
     scale_x_continuous(breaks = brs, labels = insert_minor(brs, 10)) +
     xlab("Year") +
     ylab("Title count (n)")
...
p <- ggplot(df, aes(x = publication_decade, y = paper)) +
       ...
       ggtitle("Paper consumption on Octavo") +
...
df1$publication_place <- droplevels(factor(df1$publication_place))
...
df2$publication_place[!df2$publication_place == "Stockholm"] <- "Other"
