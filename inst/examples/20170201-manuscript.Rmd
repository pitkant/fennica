...
for (catal in unique(df00$catalog)) {
  ...
  #p <- p + scale_fill_manual(values = c(cols[[catal]], "darkgray"))
  #p <- p + scale_color_manual(values = c(cols[[catal]], "darkgray"))
  ...
  p <- p + #ggtitl(paste("Paper consumption (", catal, ")", sep = "")) +
         #ggtitle(paste("Figure 3", c("A", "B")[which(unique(df00$catalog) == catal)], sep = "")) +
         ggtitle(paste(c("A", "B")[which(unique(df00$catalog) == catal)], sep = "")) +       	 
...
grid.arrange(pics[[1]], pics[[2]], top = "Figure 3", layout_matrix = matrix(c(1,2), nrow = 2, byrow=TRUE))
```
...
```{r Fig4, echo=FALSE, message=FALSE, warning=FALSE, cache=FALSE, fig.height=4, fig.width=12}
cols <- list(Fennica = "blue", Kungliga = "darkred")
