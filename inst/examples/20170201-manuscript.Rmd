...
```{r LIBER-13, echo=FALSE, message=FALSE, warning=FALSE, echo=FALSE, fig.width=11, fig.height=9, fig.show="hold", out.width="240px"}
...
p <- ggplot(df, aes(x = publication_decade, y = mean_title_length,
       ...
       geom_point(aes(size = n, shape = language)) +
...
p <- ggplot(df, aes(x = publication_decade, y = paper)) +
       geom_line(aes(linetype = publication_place), size = 1) +
       geom_point(aes(shape = publication_place), size = 2) +
       xlab("Publication year") +
       ylab("Paper (sheets)") +
