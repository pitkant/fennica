...
insert_minor <- function(major, n) {
  minor <- major
  minor[-seq(1, length(major), n)] <- ""
  minor
}
...
brs <- seq(min(df$publication_decade), max(df$publication_decade), 20)
...
for (varname in c("n", "paper", "publishers")) {
  ...
  p <- p + scale_x_continuous(breaks = brs, labels = insert_minor(brs, 5)) 
