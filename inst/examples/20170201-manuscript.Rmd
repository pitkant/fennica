...
# knitr::opts_chunk$set(dev = "Cairo")
...
for (catal in unique(df.full$catalog)) {
  ...
  df2 <- subset(df00, catalog == catal) %>%
	   ...
	   summarize(paper = sum(paper, na.rm = TRUE)/1e6, n = n())
  # Million sheets	   
  ...
  p <- p + xlab("Decade")
  p <- p + ylab("Paper (million sheets)")
  # p <- p + ylim(c(0, max(na.omit(df00$paper))))
  ...
  p <- p + 
	   ...
	   labels = insert_minor(brs, 5, start = ifelse(catal == "Fennica", 1500, 1550))) + 
         scale_y_continuous(breaks = seq(0, 6.8, 1), limits = c(0, 6.8))
         #scale_y_continuous(breaks = seq(0, max(df00$paper, na.rm = TRUE), 1000))
	                    # label = scientific_10)
...
#print(plot_grid(title, p, ncol=2, rel_heights=c(0.1, 1), hjust = 0))
#print(p + theme_blank(30))
print(p)
```
...
#grid.arrange(pics[[1]], pics[[2]], pics[[4]], pics[[5]], layout_matrix = matrix(c(1,2,3,4), nrow = 2, byrow=TRUE))


library(cowplot)
# arrange the three plots in a single row
prow <- plot_grid(
           pics[["Fennica-n"]] + theme(legend.position="none"),
           pics[["Fennica-paper"]] + theme(legend.position="none"),
           pics[["Kungliga-n"]] + theme(legend.position="none"),
           pics[["Kungliga-paper"]] + theme(legend.position="none"),
           align = 'vh',
           # labels = c("A", "B", "", ""),
	   # label_size = 25,	   
           hjust = -1,
...
npub0 <- df %>% group_by(publication_year, publisher) %>% tally() %>% arrange(n)
...
#grid.arrange(pics[[1]], pics[[8]], pics[[9]], pics[[10]], layout_matrix = matrix(c(1,2,3,4), nrow = 2, byrow=TRUE))

library(cowplot)
# arrange the three plots in a single row
prow <- plot_grid(
           pics[["Fennica-Turku"]] + theme(legend.position="none"),
           pics[["Kungliga-Stockholm"]] + theme(legend.position="none"),
           pics[["Kungliga-Uppsala"]] + theme(legend.position="none"),
           pics[["Kungliga-Lund"]] + theme(legend.position="none"),	   	  
           align = 'vh',
           # labels = c("A", "B", "", ""),
	   # label_size = 25,	   
           hjust = -1,
...
#print(p + theme_light(scalingfactor))
print(p)
```
...
s <- skim(subset(df0, catalog == "Fennica")[, c("publication_year", "publication_place", "gatherings", "title", "pagecount", "publisher", "language", "subject_topic")])
