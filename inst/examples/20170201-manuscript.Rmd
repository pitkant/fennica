...
knitr::opts_chunk$set(fig.path = "20170201_manuscript/", dpi = 300)
knitr::opts_chunk$set(dev = "jpeg", quality = 100) 
#knitr::opts_chunk$set(dev = "cairo_pdf") # Embed fonts
#knitr::opts_chunk$set(dev = "cairo_ps") # Embed fonts
# knitr::opts_chunk$set(dev = c("png")) # Embed fonts
knitr::opts_chunk$set(out.width = "10cm") # Default size 8.5cm ie 3.35 inch) per col
...
```{r Figure_1, message=FALSE, warning=FALSE, fig.width=11, fig.height=5, echo=FALSE}
...
#postscript(file = "Figure_1.eps", width = 11, height = 5, paper = "special",
#              family = "sans", bg = "white")
#print(p)
#dev.off()
#library(extrafont)
#embed_fonts("Figure_1.eps", outfile="Figure_1.eps")
```
...
```{r Figure_2, message=FALSE, warning=FALSE, fig.width=10, fig.height=4, echo=FALSE}
library(dplyr)
library(ggplot2)
theme_set(theme_bw(20))
df <- df0 %>% filter(publication_year >=1640 & publication_year <=1828) %>%
...
```{r Figure_3prep, echo=FALSE, message=FALSE, warning=FALSE, echo=FALSE, fig.width=18, fig.height=5, fig.show="hide", results="hide"}
library(scales)
...
```{r Figure_3, echo=FALSE, message=FALSE, warning=FALSE, echo=FALSE, fig.width=18, fig.height=5}
print(plot_grid(title, p, ncol=1, rel_heights=c(0.1, 1), hjust = 0))
```
...
```{r Figure_4, echo=FALSE, message=FALSE, warning=FALSE, cache=FALSE, fig.height=5, fig.width=15, fig.show="last"}
cols <- list(Fennica = "blue", Kungliga = "darkred")
...
```{r Figure_5, echo=FALSE, message=FALSE, warning=FALSE, echo=FALSE, fig.width=15, fig.height=7}
pics <- list()
...
```{r Figure_6, echo=FALSE, message=FALSE, warning=FALSE, fig.width=10, fig.height=4}
library(magrittr)
library(ggplot2)
library(dplyr)
v <- seq(1500, 1800, 100)
...
```{r Figure_7, echo=FALSE, message=FALSE, warning=FALSE, fig.width=11, fig.height=4}
...
```{r Figure_8, echo=FALSE, message=FALSE, warning=FALSE, cache=FALSE, fig.height=5, fig.width=12}
...
for (dec in npub$publication_decade) {
  inds <- which(npub$publication_decade == dec & npub$publication_place == "Other")
...
```{r Figure_9, echo=FALSE, message=FALSE, warning=FALSE, cache=FALSE, fig.height=5, fig.width=10}
library(dplyr)
catalogue <- "Fennica"
place <- "Vaasa"
minyear <- 1750
...
```{r Figure_10, echo=FALSE, message=FALSE, warning=FALSE, cache=FALSE, fig.height=5, fig.width=12}
library(dplyr)
place <- "Turku"
catalogue = "Fennica"
df = df0
...
p <- ggplot(npub, aes(x = publication_decade, y = publisher)) +
      ...
      #ggtitle(paste("Figure 10: ", place, " (", catalogue, ")", sep = ""))      
...
```{r Figure_11, echo=FALSE, message=FALSE, warning=FALSE, fig.width=10, fig.height=5}
...
p <- ggplot(dfm, aes(x = publication_decade, y = titles_per_capita)) +
       ...
       #scale_color_stata() +
       #scale_color_manual(values = city.colors[as.character(levels(dfm$publication_place))]) +
...
```{r Figure_12, echo=FALSE, message=FALSE, warning=FALSE, fig.width=10, fig.height=5}
minyear <- 1640
maxyear <- 1828
...
labs <- brs; labs[!brs %in% df$publication_decade] <- ""
#df$publication_place <- factor(df$publication_place, levels = levels(df0$publication_place))
...
```{r Figure_13, echo=FALSE, message=FALSE, warning=FALSE, fig.width=10, fig.height=5}
# Take cities from Kungliga except Turku
cat <- "Kungliga"
df1 <- df0 %>% filter(publication_place %in% setdiff(cities, "Turku")) %>%
...
```{r Figure_14, echo=FALSE, message=FALSE, warning=FALSE, fig.width=10, fig.height=5}
# Take cities from Kungliga except Turku
...
```{r Figure_15, echo=FALSE, message=FALSE, warning=FALSE, fig.width=15, fig.height=7}
pics <- list()
theme_set(theme_bw(20))
...
grid.arrange(pics[[1]], pics[[8]], pics[[9]], pics[[10]], top = "Figure 15", layout_matrix = matrix(c(1,2,3,4), nrow = 2, byrow=TRUE))
...
```{r Figure_16, echo=FALSE, message=FALSE, warning=FALSE, fig.width=10, fig.height=5, fig.show="hold"}
catalogue <- "Fennica"
...
p <- ggplot(df, aes(x = publication_decade, y = n, shape = language)) +
       ...
       scale_fill_brewer(palette="Greys") +
       #scale_color_manual(values = city.colors[as.character(levels(df$publication_place))]) +
...
```{r Figure_17, echo=FALSE, message=FALSE, warning=FALSE, fig.width=10, fig.height=5}
...
```{r Figure_18, echo=FALSE, message=FALSE, warning=FALSE, fig.width=10, fig.height=5, fig.show="hold"}
...
p <- ggplot(df, aes(x = publication_decade, y = paper, group = language)) +
       ...
       #scale_color_stata() +
       #scale_color_manual(values = lang.colors[as.character(levels(df$language))]) +
       scale_color_manual(values = lang.colors[match(unique(df$language), names(lang.colors))]) +              
       # scale_x_continuous(breaks = brs, labels = insert_minor(brs, 5)) +
       #scale_x_continuous(breaks = brs, labels = insert_minor(brs, 5, start = century(min(brs)) + 50)) +
       scale_x_continuous(breaks = brs, labels = insert_minor(brs, 2)) +       
       scale_y_continuous(label = scientific_10) 
