...
for (catal in unique(df$catalog)) {
  pics[[catal]] <- ggplot(subset(df, catalog == catal),
       ...
       scale_y_continuous(limits = c(0, max(df$title_length))) +
...
p <- ggplot(npub, aes(x = publication_decade, y = n)) +
       ...
       scale_x_continuous(breaks = brs, labels = insert_minor(brs, 2)) +   
       ggtitle(paste("Unique publishers (", catal, ")", sep = ""))
...
theme_set(theme_bw(20))
brs <- seq(min(npub$publication_decade), max(npub$publication_decade), 5)  
p <- ggplot(npub, aes(x = publication_decade, y = n)) +
       geom_bar(stat = "identity", position = "stack", color = "black",
       		    aes(fill = publication_place)) + 
       xlab("Publication year") +
