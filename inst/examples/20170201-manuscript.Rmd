...
df0$title_length_nchar <- nchar(df0$title)
df0$title_length_wordcount <- sapply(strsplit(df0$title, " "), length)
df0$title_length_wordcount[is.na(df0$title)] <- NA
```
...
```{r places_per_decade, message=FALSE, warning=FALSE, fig.width=10, fig.height=4, echo=FALSE}
library(dplyr)
library(ggplot2)
...
```{r, message=FALSE, warning=FALSE, fig.width=10, fig.height=4, echo=FALSE}
library(dplyr)
library(ggplot2)
theme_set(theme_bw(20))
...
```{r LIBER-13, echo=FALSE, message=FALSE, warning=FALSE, echo=FALSE, fig.width=9, fig.height=4, fig.show="hold", out.width="400px"}
for (catal in unique(df0$catalog)) {
  ...
  df2$gatherings <- factor(df2$gatherings, levels = levels(df0$gatherings))
...
```{r publishingovertime, echo=FALSE, message=FALSE, warning=FALSE, fig.width=12, fig.height=4}
library(magrittr)
library(ggplot2)
library(dplyr)
v <- seq(1500, 1800, 100)
...
```{r riksdar_kungliga, echo=FALSE, message=FALSE, warning=FALSE, fig.width=15, fig.height=5}
...
```{r riksdar_fennica, echo=FALSE, message=FALSE, warning=FALSE, fig.width=15, fig.height=5}
minyear <- 1760
maxyear <- 1860
catal <- "Fennica"

df00 <- df.full %>% filter(publication_year >= minyear & publication_year <= maxyear)
...
library(reshape2)
  ...
  # Use timeinterval year intervals
  dfsel <- subset(df00, catalog == catal)
  ...
  rectangles$shade <- rep(c("Background", "Highlight"),
  		          length = nrow(rectangles))
...
```{r kungliga9b, echo=FALSE, message=FALSE, warning=FALSE, cache=FALSE, fig.height=7, fig.width=15}
df <- df0
catal <- "Kungliga"
df <- df %>%
        filter(catalog == catal) %>%
...
p <- ggplot() + ggtitle("Fig 10: to be added")
...
```{r publishers-fennica-11, echo=FALSE, message=FALSE, warning=FALSE, cache=FALSE, fig.height=6, fig.width=12}
library(dplyr)
place <- "Turku"
catalogue = "Fennica"
df = df0
...
```{r octavo2, echo=FALSE, message=FALSE, warning=FALSE, fig.width=10, fig.height=5}
# Take cities from Kungliga except Turku
cat <- "Kungliga"
df1 <- df0 %>% filter(publication_place %in% setdiff(cities, "Turku")) %>%
      	       filter(catalog == cat & gatherings == "8vo") %>%
...
p <- ggplot(df, aes(x = publication_decade, y = mean_title_length)) +
       ...
       ggtitle("Octavo: mean title length") +
       guides(shape = guide_legend(title = "Place")) +
       guides(linetype= guide_legend(title = "Place"))        
print(p)
```
...
```{r publishersper, echo=FALSE, message=FALSE, warning=FALSE, fig.width=12, fig.height=6, fig.show="hold"}
# Selected catalogue with selected decades
catalogue <- "Fennica"

df <- filter(df0, catalog == catalogue &
     		    publication_decade >= 1640 &
		    publication_decade <= 1828)

top.places <- names(top(df, "publication_place", 5))

df <- df %>%
      filter(publication_place %in% top.places) %>% 
      group_by(publication_decade, publication_place) %>%
      summarise(n = length(unique(na.omit(publisher))))

  theme_set(theme_bw(20))
  p <- ggplot(df, aes(x = publication_decade, y = n, shape = publication_place)) +
       geom_point(size = 5) +
       geom_line(size = 1, aes(linetype = publication_place)) +
       scale_color_brewer(palette="Greys") +
       #scale_fill_brewer(palette="Greys") +                     
       xlab("Publication year") +
       ylab("Publishers (n)") +
       guides(shape = guide_legend("Place")) +
       guides(linetype = guide_legend("Place")) +
       ggtitle(paste(catalogue))
  print(p)
