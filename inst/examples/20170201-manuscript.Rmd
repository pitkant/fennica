...
for (catal in unique(df00$catalog)) {
  ...
  p <- p + ggtitle(paste("Paper consumption (", catal, ")", sep = "")) +
	   ...
	   labels = insert_minor(brs, 5, start = century(min(df2$publication_decade)))) + 
...
for (catal in unique(df$catalog)) {
  pics[[catal]] <- ggplot(subset(df, catalog == catal),
       ...
       scale_y_continuous(limits = c(0, max(na.omit(df$title_length)))) +
...
p <- ggplot(npub, aes(x = publication_decade, y = n)) +
       ...
       scale_x_continuous(breaks = brs, labels = insert_minor(brs, 1)) +   
...
theme_set(theme_bw(20))
brs <- seq(min(npub$publication_decade), max(npub$publication_decade), 1)  
p <- ggplot(npub, aes(x = publication_decade, y = n)) +
       geom_bar(stat = "identity", position = "stack", color = "black",
       		    aes(fill = publication_place)) + 
       xlab("Publication year") +
       ylab("Unique publishers (n)") +
       scale_fill_stata() +
       scale_x_continuous(breaks = brs, labels = insert_minor(brs, 2, start = min(npub$publication_decade))) + 
       guides(fill = guide_legend(reverse = FALSE, title = "")) +
       ggtitle(paste("Publishers (", catal, ")", sep = ""))       
...
brs <- seq(min(decade(df$publication_year)), max(decade(df$publication_year)), 1)
...
df1 <- df1 %>% summarise(n = n(), paper = sum(paper, na.rm = TRUE), 
