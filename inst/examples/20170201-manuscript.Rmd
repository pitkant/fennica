...
df0$title_length <- nchar(df0$title)
```
...
p <- ggplot(df, aes(x = publication_decade, y = n, shape = catalog, linetype = catalog)) + 
       ...
       scale_color_manual(values = c("blue", "darkgreen")) + 
       guides(shape = guide_legend(title = "Catalog")) +
       guides(linetype = guide_legend(title = "Catalog")) +       
       ggtitle("Title count")
print(p)
```
...
```{r LIBER-13, echo=FALSE, message=FALSE, warning=FALSE, echo=FALSE, fig.width=7, fig.height=4, fig.show="hold", out.width="400px"}
...
p <- ggplot(df, aes(x = publication_decade, y = mean_title_length)) +
       geom_point(aes(size = n, shape = language), size = 3) +
...
library(reshape2)
  ...
  df <- subset(dfsel, publication_year >= 1700 & publication_year < 1831 )
