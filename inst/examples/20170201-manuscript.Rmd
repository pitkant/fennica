...
for (catal in unique(df$catalog)) {
  pics[[catal]] <- ggplot(subset(df, catalog == catal),
       ...
       ggtitle(paste("Median title length (", catal, ")", sep = "")) + 
       xlab("Decade") +
...
p <- ggplot(df,
       ...
       ggtitle("Average paper consumption per title")
...
library(reshape2)
  ...
  p <- p + xlab("Year")
  p <- p + ggtitle(paste("Title count (", catal, " ", minyear, "-", maxyear, ")", sep = ""))
  print(p)
...
p <- ggplot(npub, aes(x = publication_decade, y = n)) +
       ...
       scale_x_continuous(breaks = brs, labels = insert_minor(brs, 2)) +   
       ggtitle(paste("Publishers (", catal, ")", sep = ""))
