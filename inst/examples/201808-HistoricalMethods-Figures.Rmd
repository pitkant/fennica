...
#theme_set(theme_bw(20))
df <- df0 %>% filter(publication_year >=1640 & publication_year <=1828) %>%
        group_by(Catalogue, publication_decade) %>% tally()
...
for (catal in unique(df.full$Catalogue)) {
  ...
  if (catal == "SNB") {
    df00 <- df.full # df.full.1911 %>% filter(publication_year >= min.year & publication_year <= max.year)
  } else if (catal == "FNB") {
    df00 <- df.full.1911 #%>% filter(publication_year >= min.year & publication_year <= 1900)
  }
...
for (Catalogue in na.omit(unique(df0$Catalogue))) {

df <- df0 %>% filter(Catalogue == Catalogue)%>%
...
df <- df0 %>% filter(Catalogue == Catalogue & publication_place == place) %>%	
...
df <- df %>% filter(Catalogue == Catalogue & publication_place == place) %>%
...
df <- df %>% filter(Catalogue == Catalogue & hit)
  lang <- paste("language.", langs, sep = "")
  otherlang <- setdiff(names(df)[grep("lang.", names(df))], lang)
  otherlang <- otherlang[otherlang!="language.Multiple languages"]
  df$language.Other <- rowSums(df[, otherlang] == TRUE, na.rm = T) > 0
...
Catalogue <- "FNB"
  ...
  df <- filter(df0, Catalogue == Catalogue &
