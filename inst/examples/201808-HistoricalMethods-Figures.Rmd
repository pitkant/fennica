...
df.full <- df.full %>% filter(publication_year >= min.year & publication_year <= max.year)
...
# Change catalog name
df.full$catalog <- gsub("Fennica", "FNB", df.full$catalog)
df.full$catalog <- gsub("Kungliga", "SNB", df.full$catalog)
df.full$catalog <- factor(df.full$catalog)
...
df1 <- df00 %>% filter((publication_place %in% setdiff(cities, "Turku") & catalog == "SNB") |
                       (publication_place %in% "Turku" & catalog == "FNB")) 
