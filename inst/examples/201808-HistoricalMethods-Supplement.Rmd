...
fen <- subset(df0, catalog == "FNB")
kun <- subset(df0, catalog == "SNB")
 ...
 * FNB: ```r nrow(subset(df0, catalog == "FNB"))```
 * SNB: ```r nrow(subset(df0, catalog == "SNB"))```
...
In FNB, we have identified ```r sum(subset(df0, catalog == "FNB")$multilingual)``` multilingual documents in the investigated time period. In SNB, the multilingual documents have not been analyzed yet. We use the primary language of the document for summaries.
...
df <- df0 %>% group_by(catalog, gatherings) %>%
              summarise(n = n())
...
df <- df0
names(df) <- gsub("catalog", "Catalog", names(df))
df <- df %>% group_by(Catalog, publication_decade) %>% tally()
p <- ggplot(df, aes(x = publication_decade, y = n, fill = Catalog)) +
...
In FNB, the data includes ```r  length(unique(subset(df0, catalog == "FNB")$publisher))``` unique publishers, and ```r  round(100 * mean(!is.na(subset(df0, catalog == "FNB")$publisher), na.rm = TRUE))```% of the documents have publisher information. We have not analyzed publishers in SNB, where the availability of harmonized publisher information is currently low (```r  round(100 * mean(!is.na(subset(df0, catalog == "SNB")$publisher), na.rm = TRUE))```%). 
...
We use here the term "Topic" for the "Topical term" MARC field 650a for brevity. In FNB, the data includes ```r  length(unique(subset(df0, catalog == "FNB")$Topic))``` unique topics. ```r  round(100 * mean(!is.na(subset(df0, catalog == "FNB")$Topic), na.rm = TRUE))```% of the documents have topic information. We have not analyzed topics in SNB, where the availability of harmonized topics is currently low (```r  round(100 * mean(!is.na(subset(df0, catalog == "SNB")$Topic), na.rm = TRUE))```%). 
