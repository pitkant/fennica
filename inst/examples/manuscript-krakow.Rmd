...
```{r publishingovertime, echo=FALSE, message=FALSE, warning=FALSE, fig.width=8, fig.height=6}
...
Comparing printing activity in Turku, Uppsala, Lund, and
...
I can here provide percentages, barplots, selected time spans,
selected places etc.. just let me know what is needed.

```{r publishingactivitycomparisons, echo=FALSE, message=FALSE, cache=FALSE, fig.width=10, fig.height=7, fig.show="hold", out.width="430px"}
...
```{r valtiopaivat-total, echo=FALSE, warning=FALSE, fig.width=11, fig.height=4}
...
p <- p + xlab("Year")
p <- p + ggtitle("Riksdagar - overall titlecount (Fen + Kun)")
print(p)
```


```{r valtiopaivat-gavle, echo=FALSE, warning=FALSE, fig.width=11, fig.height=4}
# Use timeinterval year intervals
myplace = "Gävle" # Norrköping / Stockholm
dfsel = subset(df0, publication_place == myplace)
minyear = 1700
maxyear = 1800
...
rect_left <- c(min(na.omit(dfm$date)),
  ...
  rectangles <- data.frame(
    xmin = rect_left[-length(rect_left)],
    xmax = rect_left[-1],
    #xmin = min(dfsel$publication_year),
    #xmax = max(dfsel$publication_year),    
    ymin = ymin,
    ymax = ymax
    )
  rectangles$shade <- rep(c("White", "Highlight"), length = nrow(rectangles))
...
p <- p + ylab("Documents / Year")
p <- p + xlab("Year")
p <- p + ggtitle(paste("Riksdagar - titlecount in ", myplace, sep = ""))
print(p)
...
```{r 201606krakow-fennica-topfemale, echo=FALSE, message=FALSE, warning=FALSE, cache=FALSE, fig.height=5, fig.width=15, out.width="850px"}
