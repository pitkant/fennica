...
df2 <- df0 %>% group_by(publication_decade, catalog) %>% summarise(paper = sum(paper.consumption.km2, na.rm = TRUE))
p <- ggplot(df2, aes(x = publication_decade, y = paper)) +
...
p <- p + geom_line(data = dfm, aes(x = date, y = Documents), col = "black")
p <- p + geom_point(data = dfm, aes(x = date, y = Documents), col = "black")
p <- p + scale_x_continuous(breaks=seq(min(dfsel$publication_year), max(dfsel$publication_year), 20))
p <- p + ggtitle("Publishing activity")
p <- p + ylab("Documents / Year")
p <- p + xlab("Year")
