...
Records in the final data set used in the analyses (years ```r min(df0$publication_year)```-```r max(df0$publication_year)```):
...
Percentage of non-empty entries for each field (per catalogue).
...
Number of unique and all genres per catalogue.
...
kable(top(fen, "Subject", 10, "data.frame"), digits = 1)
```
...
kable(top(kun, "Subject", 10, "data.frame"), digits = 1)
```


### Publisher

Number of unique and all publishers per catalogue.
...
kable(top(fen, field, 10, "data.frame"), digits = 1)
```
...
kable(top(kun, field, 10, "data.frame"), digits = 1)
```
...
Number of unique and all authors per catalogue.
...
```{r pagecount, echo=FALSE, message=FALSE, warning=FALSE, fig.width=10, figh.height=3}
p <- ggplot(df0) +
...
```{r paper, echo=FALSE, message=FALSE, warning=FALSE, fig.width=10, figh.height=3}
p <- ggplot(df0) +
...
Number of unique and all languages per catalogue.
...
Percent of unique and all gatherings per catalogue.
