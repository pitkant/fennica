...
dfs <- df %>% select(publication_place, country, published_in, published_from, published_till, corporate, area, pagecount, gatherings, width, height, paper.consumption.km2)
missing <- 100*apply(dfs, 2, function (x) {mean(is.na(gsub("^NA$", NA, as.character(x))))})
df2 <- data.frame(list(missing = missing, field = names(missing)))
df2$field <- factor(df2$field, levels = df2$field[rev(order(df2$missing))])
...
dfs <- df.orig %>% select(publication_place, note_year, corporate, physical_extent, physical_dimension)
missing <- 100*apply(dfs, 2, function (x) {mean(is.na(gsub("^NA$", NA, as.character(x))))})
df2 <- data.frame(list(missing = missing, field = names(missing)))
df2$field <- factor(df2$field, levels = df2$field[rev(order(df2$missing))])
...
p <- p + ylab("")
p <- p + xlab("")
p <- p + ggtitle("Documents with raw data (%)")
print(p)
```
