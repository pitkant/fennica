 ...
 * `r sum(!is.na(df$title))`/`r nrow(df)` original documents (`r round(100*mean(!is.na(df$title)), 1)`%) have data
 * `r sum(is.na(df$title))`/`r nrow(df)` documents (`r round(100*mean(is.na(df$title)), 1)`%) have missing (NA) entry
