...
#organize data by unique title 
#add frequency column 
```{r}
df.tmp<- transform(df.tmp, freq= ave(seq(nrow(df)), title, FUN=length))
df.tmp[order(-df.tmp$freq), ]
na.omit(df.tmp)
```

```{r uniquetitles, echo=FALSE, message=FALSE, warning=FALSE, fig.width=8, fig.height=6}
library(ggplot2)
p <- ggplot(df.tmp, aes(freq, color = "black"))+ geom_bar(width=0.05, colour="blue")+scale_x_log10()+ scale_y_log10()
print(p) 
```